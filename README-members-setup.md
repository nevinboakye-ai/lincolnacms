...
## 73. Going-live polish: no more em dashes, banner reworded, homepage cards reordered, real social links

No migration needed — front-end only, across every page.

- **Every em dash on the site swapped for a plain hyphen.** A fresh sweep, since real content added since the last pass (committee bios, various fixes) could in principle have reintroduced one — turned out everything since then was already written with plain hyphens, so this pass changed nothing, just confirmed it stayed clean.
- **The construction-notice banner is retired.** `"From Nevin: The website is still under construction; my apologies for any errors."` → **"Website built by Nevin Boakye"**, on all 20 pages — the site's going live, so an apology for it being unfinished no longer fits; a plain attribution does.
- **Homepage impact cards reordered**: Discounts & Opportunities, then The ACMS Midlands Medics Gala, then Sankofa Mentorship, then Discover the LACMS Network, then Our Impact — a pure DOM reorder, each card's own CSS (colours, shimmer delay) is keyed to its class name, not its position, so nothing else needed to change to make the new order render correctly.
- **About page's "Our values" cards no longer say `[Placeholder]`** — Belonging, Mentorship, Excellence and Culture all read as finished copy now, matching the "Our story" section fixed earlier.
- **Instagram and TikTok buttons go somewhere now.** Every `.social-btn` IG/TikTok pair, across every page footer (40 links total), pointed at `href="#"` — now `https://www.instagram.com/lincolnacms/` and `https://www.tiktok.com/@lincolnacms`, both `target="_blank" rel="noopener"` to match how the existing LinkedIn button already opens externally.

## 74. Header banner shortened, homepage course list, Opportunities' "Learn more" is now a real gate

No migration needed — front-end only.

**The construction-notice banner is gone**, replaced with a plain "Built by Nevin Boakye" across every page — reasonable now that the site's actually launching, rather than apologising for it still being unfinished.

**Homepage course line**: "For students of Healthcare Professions-regulated courses" → "For students of healthcare-related courses" (the label is already uppercased by CSS — `.hero-courses-label`'s own `text-transform`, so the HTML just holds normal sentence case, same as every other eyebrow-style label on the site). **Biomedical Science** added to the course list underneath, between Dental Hygiene and Therapy and Diagnostic Radiography.

**Opportunities page's "Learn more" was quietly linking to whatever happened to be in each row's own `link` field** (real, member-perks-only-style external URLs) or rendering nothing at all if that field was empty — never actually gated on being signed in, and not really meant to be public-facing in the first place. It's now a deliberate, deterministic gate instead of an accidental one: every row always shows a "Learn more" button (a real `<button>`, not a link anymore); signed out, clicking it sends someone to `login.html` (the same account-type chooser this page's own "Sign in to see more" gate already uses, for consistency); signed in, since the fuller detail view this button is meant to eventually open isn't built yet, it says so plainly — the button's own label swaps to "Coming soon" for a couple of seconds rather than linking out to something that was never meant to be a public destination. `member-perks.html`'s own members-only opportunities list (`renderOpportunityCard`) is untouched — that one still links straight to the real URL, since it's genuinely member-gated already and the link there is the actual point.

## 75. Events page dropdowns rewritten, and em dashes fixed in seeded opportunity/news text

Run [`db/migrations/039-fix-em-dashes-in-seed-text.sql`](db/migrations/039-fix-em-dashes-in-seed-text.sql).
